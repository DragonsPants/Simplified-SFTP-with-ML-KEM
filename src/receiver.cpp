...
private:
    ...
    */
    ...
    bool SendEncapsKey(const std::vector<Byte>& data);
    bool ReadCyphertext(std::vector<Byte>& data);
    bool InitSecretKey();
...
bool FileReceiver::SendEncapsKey(const std::vector<Byte>& data) {
    
    size_t fileSize = data.size();
    int bytesSent = send(clientSocket, &fileSize, sizeof(fileSize), 0);
    if (bytesSent < 0) {
        Log::Error("SendEncapsKey()","Size Send Failed");
        return false;
    }
    
    int sentBytes = send(clientSocket, data.data(), data.size(), 0);
    if (sentBytes < 0) {
        Log::Error("SendEncapsKey()","Encaps Key Send Failed");
        return false;
    }
...
bool FileReceiver::ReadCyphertext(std::vector<Byte>& data){
    size_t fileSize = -1;
    std::vector<Byte> cyphertext;

    if (read(clientSocket, &fileSize, sizeof(fileSize)) != sizeof(fileSize)) {
        Log::Error("ReadCyphertext()", "Error reading file size");
        return false;
    }
    Log::Success("Recieved Info", std::format("Text size {}", fileSize));

    std::string buffer(32768, '\0');
    int bytesRead = read(clientSocket, &buffer[0], fileSize);
    if (bytesRead < 0) {
        Log::Error("CreateSecretKey()", "Error reading Encaps key data");
        return false;
    }
    cyphertext.insert(cyphertext.end(), buffer.begin(), buffer.begin() + bytesRead);
    Log::Success("Recieved Info", "Cyphertext");

    if (bytesRead != fileSize) {
        Log::Error("CreateSecretKey()", std::format("File size mismatch, expected {} bytes, but read {} bytes", fileSize, bytesRead));
        return false;
    }
...
bool FileReceiver::InitSecretKey(){
    std::vector<Byte> encaps_key;
    std::vector<Byte> decaps_key;
    std::vector<Byte> cyphertext;

        //perform key gen
    bool keygenSuccess = Crypto::ML_KEM_KeyGen( encaps_key, decaps_key );
    if ( !keygenSuccess ){
        Log::Error("AcceptConnection()","Key Gen Failed");
    }

        //send key to Client
    SendEncapsKey(encaps_key);

        //retrieve cyphertext from Client
    ReadCyphertext(cyphertext);

        //perform decaps
    if ( !Crypto::ML_KEM_Decaps( decaps_key, cyphertext, secret_key ) ){
        Log::Error("AcceptConnection()","Decaps Failed");
    }
...
*/
    //RYAN - added call to InitSecretKey
...
bool FileReceiver::ReceiveFile(const std::string& filename) {
    ...
    // -- Step 2 --
    // Decrypt the Data
    bool decryptionStatus = Crypto::DecryptData(encryptedData, decryptedData, secret_key);
