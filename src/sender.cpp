...
/* RYAN
    I am responsible for creating CreateSecretKey,
    ReadEncapsKey, and SendDecapsKey
*/
...
private:
    ...
    */
    ...
    bool ReadEncapsKey( std::vector<Byte>& data );
    bool SendDecapsKey(const std::vector<Byte>& data);
...
bool FileSender::SendDecapsKey(const std::vector<Byte>& data) {
    size_t fileSize = data.size();
    int bytesSent = send(socketFD, &fileSize, sizeof(fileSize), 0);
    if (bytesSent < 0) {
        Log::Error("SendDecapsKey()","Size Send Failed");
        return false;
    }
    
    int sentBytes = send(socketFD, data.data(), data.size(), 0);
    if (sentBytes < 0) {
        Log::Error("SendDecapsKey()","Decaps Key Send Failed");
        return false;
    }
...
bool FileSender::ReadEncapsKey( std::vector<Byte>& data ){
    size_t fileSize = -1;
    std::vector<Byte> encaps_key;

    if (read(socketFD, &fileSize, sizeof(fileSize)) != sizeof(fileSize)) {
        Log::Error("CreateSecretKey()", "Error reading file size");
        return false;
    }
    Log::Success("Recieved Info", std::format("KeySize {}", fileSize));

    std::string buffer(32768, '\0');
    int bytesRead = read(socketFD, &buffer[0], fileSize);
    if (bytesRead < 0) {
        Log::Error("CreateSecretKey()", "Error reading Encaps key data");
        return false;
    }
    encaps_key.insert(encaps_key.end(), buffer.begin(), buffer.begin() + bytesRead);
    Log::Success("Recieved Info", "Encaps Key");
...
*/
//RYAN - added call to CreateSecretKey
...
bool FileSender::EncryptAndSend(const std::vector<Byte>& plainFileData) {
    ...
    */
    ...
    bool encryptionStatus = Crypto::EncryptData(plainFileData, encryptedData, secret_key);
